...
for name, param in net.bert.encoder.layer[11].named_parameters():
    param.requires_grad = True

for name, param in net.bert.encoder.layer[10].named_parameters():
    param.requires_grad = True

optimizer = optim.SGD([
    # BERTの11, 10層目
    {'params':net.bert.encoder.layer[10].parameters(), 'lr':0.001},
    {'params':net.bert.encoder.layer[11].parameters(), 'lr':0.001},
    {'params':net.cls.parameters(), 'lr':0.001}])
...
for ep in range(30):
    ...
    for xs, ys in dataloader:
        ...
        optimizer.step()
        i += 1
    outfile = "../outputs/doccls5-" + str(ep) + ".model"
