...
# +バッチサイズ指定 : DataLoaderを設定し、Paddingの処理を行えるようにする
# DataLoader
...
class DocCls(nn.Module):
    def __init__(self,bert):
        ...
        # x2を追加：input_ids=x1, attention_mask=x2（Padding部分）
    def forward(self,x1,x2):
        bout = self.bert(input_ids=x1, attention_mask=x2)
        # バッチサイズの取り出し
        bs = len(bout[0])
        # バッチ内i番目の文に対する[CLS]（全ての入力の先頭につく、分類問題のためのトークン）の埋め込み表現
...
for ep in range(30):
    ...
    for xs, ys in dataloader:
        ...
        # pad_sequence:長さの違うテンソルを与えると, 短いものの末尾にゼロ埋めを施して次元を揃えてくれる関数
        ...
        optimizer.step()
        i += 1
    outfile = "../outputs/doccls2-" + str(ep) + ".model"
